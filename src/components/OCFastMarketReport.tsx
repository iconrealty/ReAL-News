...
type OCFastTab = 'detached' | 'attached' | 'comparison' | 'history' | 'takeaways';
...
export const OCFastMarketReport: React.FC<OCFastMarketReportProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<OCFastTab>('detached');
  ...
  const handlePropertyTypeChange = (type: 'detached' | 'attached') => {
    setPropertyType(type);
    if (activeTab === 'detached' || activeTab === 'attached') {
      setActiveTab(type);
    }
  };

  // Top 3 Metric Lookups from canonical dataset to ensure 100% data congruence
  ...
  const handleShareReport = () => {
    if (navigator.clipboard) {
      ...
      if (onShowToast) {
        onShowToast("OC Fast Report link copied to clipboard!");
      }
    }
  };
  ...
  // Render individual metric card with JULY as the principal number, mini-pills for 2025 and % change, and YTD footer
  const renderMetricCard = (metric: OCFastMetricItem, propertyType: 'detached' | 'attached') => {
    const isDOM = metric.key === 'dom';
    const isInventory = metric.key === 'inventory' || metric.key === 'months_supply';
    const isPrice = metric.category === 'pricing';
    ...
    return (
      ...
      >
        ...
        {/* Bottom Bar: Year to Date (YTD) Performance */}
  ...
  const renderMetricGroup = (title: string, metrics: OCFastMetricItem[], propertyType: 'detached' | 'attached') => {
    return (
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
          {metrics.map(m => renderMetricCard(m, propertyType))}
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-fade-in font-sans">
      ...
      {/* Main Report Tabs Bar - Matching Orange County Market Trends Format */}
      <div className="bg-white border border-slate-200/90 rounded-2xl p-2 shadow-xs">
        <div className="flex items-center space-x-1 overflow-x-auto scrollbar-none">
          {[
            { id: 'detached', label: 'Detached (Single-Family)', icon: <Home className="w-4 h-4 mr-1.5" /> },
            { id: 'attached', label: 'Attached (Condos & Townhomes)', icon: <Building2 className="w-4 h-4 mr-1.5" /> },
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  if (tab.id === 'detached' || tab.id === 'attached') {
                    setPropertyType(tab.id);
                  }
                }}
      ...
      {/* TAB 1: DETACHED SINGLE FAMILY HOMES */}
      {activeTab === 'detached' && (
        <div className="space-y-6 sm:space-y-8 animate-fade-in">
          ...
          <div className="bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-7 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center space-x-3 bg-slate-50 p-3 rounded-2xl border border-slate-200/80 text-xs shrink-0">
              ...
              <div>
                ...
                <span className="font-bold text-slate-700">July 2025 & YTD</span>
              </div>
            </div>
          </div>
          ...
          {renderMetricGroup(
            "Inventory & Supply Dynamics",
            detachedData.filter(m => m.category === 'inventory'),
            'detached'
          )}
      ...
      {/* TAB 2: ATTACHED CONDOS & TOWNHOMES */}
      {activeTab === 'attached' && (
        <div className="space-y-6 sm:space-y-8 animate-fade-in">
          ...
          {renderMetricGroup(
            "Inventory & Supply Dynamics",
            attachedData.filter(m => m.category === 'inventory'),
            'attached'
          )}
      ...
      {/* TAB 3: SIDE-BY-SIDE COMPARISON TABLE */}
          ...
          <div className="bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-8 shadow-xs space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div>
                ...
                <p className="text-xs sm:text-sm text-slate-600 font-medium">
                  Detailed side-by-side performance of Detached Single-Family vs Attached Condominiums for July 2026 and Year-to-Date.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-base font-black text-slate-900">Detached Single-Family Homes</h3>
              </div>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    ...
                    <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 font-semibold text-[11px]">
                      ...
                      <th className="p-2.5 text-right font-bold border-l border-slate-200">Thru 7-2025</th>
                      <th className="p-2.5 text-right font-bold text-slate-900">Thru 7-2026</th>
                      <th className="p-2.5 text-center font-bold text-slate-900">% Change</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {detachedData.map((row) => (
                      <tr key={row.key} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="p-3.5 text-right font-mono text-slate-600 border-l border-slate-100">{row.ytd2025 || '—'}</td>
                        <td className="p-3.5 text-right font-mono text-slate-950 font-black text-sm">{row.ytd2026 || '—'}</td>
                        <td className="p-3.5 text-center">
                          {row.ytdChange ? renderChangeBadge(row.ytdChange, row.ytdChangeNumeric, row.key === 'dom') : '—'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="space-y-3 pt-4 border-t border-slate-100">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-base font-black text-slate-900">Attached Condominiums & Townhomes</h3>
              </div>
              <div className="overflow-x-auto scrollbar-none rounded-2xl border border-slate-200">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
      ...
      {/* TAB 4: 14-YEAR PRICE HISTORY (2012–2026) */}
      {activeTab === 'history' && (
